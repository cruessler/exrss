defmodule ExRssWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_ex_rss_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ExRssWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  ## `on_mount` arguments
    ...
    * `:ensure_authenticated` - Authenticates the user from the session, and
      assigns the current_user to socket assigns based on user_token. Redirects
      to login page if there's no logged user.

    * `:redirect_if_user_is_authenticated` - Authenticates the user from the
      session. Redirects to signed_in_path if there's a logged user.
  ...
  current_user:
      ...
      defmodule ExRssWeb.PageLive do
        use ExRssWeb, :live_view

        on_mount {ExRssWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ExRssWeb.UserAuth, :ensure_authenticated}] do
  ...
  defp signed_in_path(_conn), do: ~p"/feeds"
end
